...
def get_image(path, height, width, preprocess_fn):
    ...
    image = tf.image.decode_png(img_bytes, channels=3) if png else tf.image.decode_jpeg(img_bytes, channles=3)
    return preprocess_fn(image, height, width)

def image(batch_size, heigth, width, path, preprocess_fn, epochs=2, shuffle=True):
    ...
    processed_image = preprocess_fn(image, heigth, width)
